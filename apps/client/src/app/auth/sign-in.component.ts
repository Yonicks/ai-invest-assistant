...
@Component({
  ...
  template: `
    <form [formGroup]="form" (ngSubmit)="onSubmit()" class="max-w-sm mx-auto mt-16 bg-white shadow-lg rounded-xl p-8 space-y-6">
      ...
      <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg" [disabled]="form.invalid">Login</button>
    </form>
  `
})
export class SignInComponent {
  ...
  onSubmit() {
    ...
    this.auth.login(email!, password!).subscribe({
      next: user => {
        // Save user in store/localStorage as needed
        this.error = null;
